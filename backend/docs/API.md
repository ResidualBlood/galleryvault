...
| GET | `/api/notifications` | In-app notification ring (process memory, maxlen 100, dropped on restart): `{items: [{id, kind, title, detail, created_at, read}], unread_count}`. Kinds: `download_ok` / `download_fail` / `download_updated` / `scan_ok` / `scan_fail` / `cookie`. Written even when Telegram is off/unconfigured. |
