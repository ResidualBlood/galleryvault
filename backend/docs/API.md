...
| GET | `/api/settings/cookie-health` | Probe or return current ExHentai Cookie login status (`{state, detail, checked_at}`). |
...
| POST | `/api/favorites/add` | Body `{gid?: int, token?: str, target_favcat: int, note?: str, items?: [{gid, token, title, note}]}`. Add galleries to an ExHentai favorite folder (`gallerypopups.php?act=addfav`) and update local `favorite_items` upon cloud confirmation. Returns `{gids, target_favcat, cloud_ok, cloud_added, cloud_failed, successful_gids, local_added}`. |
