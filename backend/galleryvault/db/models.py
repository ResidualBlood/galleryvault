...
class Gallery(Base):
    ...
    local_rating: Mapped[int | None] = mapped_column(Integer)
...
class LocalListItem(Base):
    __tablename__ = "local_list_items"
    list_id: Mapped[int] = mapped_column(
        ForeignKey("local_lists.id", ondelete="CASCADE"), primary_key=True
    )
    gallery_id: Mapped[int] = mapped_column(
        ForeignKey("galleries.id", ondelete="CASCADE"), primary_key=True
    )
    __table_args__ = (Index("idx_local_list_items_gallery_id", "gallery_id"),)
...
class Series(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, Identity(), primary_key=True)
    name: Mapped[str] = mapped_column(Text)
    match_key: Mapped[str | None] = mapped_column(String(512), unique=True, nullable=True)
    name_manual: Mapped[bool] = mapped_column(
...
class SeriesItem(Base):
    ...
    gallery_id: Mapped[int] = mapped_column(
        ForeignKey("galleries.id", ondelete="CASCADE"), primary_key=True, unique=True, nullable=False
    )
    source: Mapped[str] = mapped_column(String(16), default="auto", server_default=text("'auto'"))
    __table_args__ = (Index("idx_series_items_series_id", "series_id"),)


class SeriesExclusion(Base):
    __tablename__ = "series_exclusions"
    gallery_id: Mapped[int] = mapped_column(
        ForeignKey("galleries.id", ondelete="CASCADE"), primary_key=True, nullable=False
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
