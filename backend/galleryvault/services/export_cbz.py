...
from ..scanners.ehviewer import strip_gid_prefix
...
def cbz_filename(title: str | None, gid: int | None, gallery_id: int) -> str:
    cleaned = strip_gid_prefix(title or "", gid) if title else ""
    base = cleaned.strip() or (str(gid) if gid else f"gallery-{gallery_id}")
