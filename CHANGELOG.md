...
- **Global pause (persistent) + GP/Quota cache + Recycle Bin + Missing pages体检** (`config.py`, `app/schemas.py`, `app/routers/tasks.py`, `app/routers/galleries.py`, `db/models.py`, `db/repositories/galleries.py`, `services/deletion.py`, `services/download_worker.py`, `services/scan_worker.py`, `services/telegram_bot.py`, `frontend/assets/views/downloads.js`, `frontend/assets/views/recycle.js`, `frontend/assets/views/integrity.js`, `frontend/assets/core.js`, `frontend/assets/locales/`): Added `global_paused` (persisted via `app_config.user_settings`, `POST /api/pause` + `GET /api/pause`, download worker claim pause + scan pause 423, Telegram `/pause`/`/resume` sync same flag, top banner + downloads page toggle; `GET /api/quota` cached GP balance 30 min TTL (low-frequency, not per-page); `Gallery.trashed`/`trashed_at` + `alembic 0027` + `POST /api/galleries/{id}/favorite` deprecated note, `GET /api/galleries/trash`/`expunged`/`integrity` + `POST /api/galleries/restore`/`purge`, `GET /api/galleries/integrity` page_count vs `gallery_pages`/`file_count` (excludes `max_pages` truncation), frontend Recycle/Integrity pages with `selRecycle`/`selIntegrity`.
