...
- **P2 skeleton/cover UX** (`frontend assets/components.js:46`, `assets/utils.js:203`, `assets/views/library.js:32`, `assets/styles.css:74`): `library` initial uses `renderSkeleton(8)`; `galleryCard` no-cover uses `var(--panel-2)` placeholder with `t("noCover")` fallback; `input:focus` now shows `var(--focus-ring)` like `.btn`.
