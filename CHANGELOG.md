...
- **Configurable runtime user permissions via PUID/PGID** (`entrypoint.sh`, `docker-compose.yml`, `docs/wiki/Deployment.md`): Backend container now defaults to running directly as `root (0:0)` for zero-configuration startup when unconfigured, while supporting custom user/group privilege dropping when `PUID` and `PGID` environment variables (e.g. `1000:1000`) are provided.
