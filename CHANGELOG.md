...
- 前端中英 locale 键对齐，去掉设置/分页等硬编码文案。

### Changed

- README 中英精简为定位 + 页面表 + 可运行命令；`ENCRYPTION_KEY` 改为任意长随机串；`./Archive` 标明 compose 默认注释。
- Wiki 按当前 SPA 路由重写：顶栏含 Series；管理 tab 为独立 hash（`#/duplicates`、`#/duplicates/favorites`、`#/duplicates/cross-gid`、`#/integrity`、`#/archive`）；Home 死链改到分册。
- 缩略图端点 O(1) 旁路、SW cache v5、画廊无限滚动；worker 使用独立 DB 连接池。

### Fixed

- `repair_cbz_filenames.py` 文档不再写成容器内 `/app/scripts/`（该脚本不在镜像里）。
- CHANGELOG 补 `[1.10.0]`，打薄误记进 1.10.1 的 1.8/1.9 特性。

## [1.10.1] - 2026-09-10

系列、跨 GID、缺页体检、幻灯片、多根冷归档已在 1.8.x / 1.9.x 发布，不在此重复。

### Changed

- 下载进度查询扩大连接池，并在超时时回退。
- 前端 Service Worker 升到 v4，静态资源加 `?v=` 避免旧缓存。
- 前后端日志整理：系统日志最新在上。

### Fixed

- 下载写入校验 GIF / WebP，避免截断图落盘。
- 停机时排空客户端连接，worker 协同缩容。
- CI：series 脚本检查容忍 asset 版本 query；py7zr 升到 1.1.3。

### Dependencies

- ruff、pypdf、alembic、actions/cache 按 dependabot 更新。

## [1.10.0] - 2026-09-09

### Added

- 已归档源目录清理：设置存储表与 `#/archive` 可删热目录解压源（跳过 pending/downloading）。
- 本地分类修复：`POST /api/tag-sync/repair-categories`，不联网改 Misc/Other。
- 设置页存储表增加画廊 / 图片 / 缩略图计数。
- 离线脚本：`backend/galleryvault/scripts/repair_cold_archives.py`（容器内可用）、仓库根 `scripts/repair_cbz_filenames.py`（仅宿主）。
- ExHentai 302 自动全局暂停、Telegram 告警、后台探针恢复。

### Changed

- 后端拆 service / UoW；后台 worker 缩短持有 DB 会话的时间。
- `archive_fallback` 入库，任务列表不再 N+1 探 `.archive.json`。
- 源目录清理改为后台任务。

### Fixed

- 分类误落入 Misc；CBZ 混合扩展名；慢 H@H 节点 `skip_hath` 轮换。
- 截断图容忍与坏页修复；下载完成后才发「有更新」通知。
- ComicInfo Writer 超长截断；扫库包含归档根；空的 download-retry-sweep 不写历史。
- `.gitignore` 忽略 `HANDOFF.md` / `AGENTS.md`。
...
- **Documentation and API sync** (`docs/wiki/`, `backend/docs/`): wiki/API 补系列与 notifications/clear、FAQ 对齐 OPDS/CBZ。
