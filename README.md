...
- **Telegram 通知**：下载成功/失败、扫描完成、收藏同步时发送通知；默认**汇总**模式（批量下载合并为一条摘要），可切"即时/仅失败/关闭"。
...
2. **收藏夹读取收藏**：「收藏夹」页先「同步收藏夹名称」，再「立即检查所有」——把收藏的画廊元数据（标题/标签/封面/大小）缓存进数据库，并记录收藏集合（后续下载判断的基础）。
3. **扫描库**：把已有画廊放进库目录（`./library`、`./downloads` 等），点「扫描库」入库。步骤 2 的元数据缓存会被直接复用，标签同步快得多。
4. **查重**：同一作品多版本（DL 版/无修正/不同语言）用「收藏夹管理 → 扫描重复画廊」分组处理；多目录里的同 gid 副本用「重复副本」页按策略清理。
5. **开启自动下载**：设置里打开 **download favorites** 总开关；收藏夹页勾选要监控的文件夹、模式选「增量下载」、间隔按需（如 10 分钟）。之后**新加进收藏夹的画廊会自动下载**。
6. **存量补下（可选）**：已在收藏夹但本地还没有的画廊，把模式临时切到「强制下载」→「立即检查」一次性拉下（已在本地的自动跳过），下完切回「增量下载」。
