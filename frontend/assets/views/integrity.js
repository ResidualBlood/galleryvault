      ...
      el.innerHTML = `<div class="grid gc-grid">` + data.items.map(g => {
        ...
        return `<div class="gc-wrap">
          <a class="gc" href="${navHash("gallery", { id: g.id }, { from: currentFromPath() })}">
            <div class="gc-cover">${g.cover_url ? `<img class="cover-bg" loading="lazy" src="${esc(g.cover_url)}" alt="" aria-hidden="true"><img class="cover-fg" loading="lazy" src="${esc(g.cover_url)}" alt="">` : `<div class="cover-placeholder">${esc(t("noCover"))}</div>`}<span class="gc-pages">${mismatch}</span></div>
