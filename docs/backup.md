...
> 恢复会覆盖当前数据库内容。若备份早于启用加密，恢复后按 [静态加密](encryption.md) 重新设置 `ENCRYPTION_KEY` 即可。

## 备份中的密钥

启用 [静态加密](encryption.md) 后，数据库备份里的 cookie / token 是密文。**备份文件不包含密钥**——请把 `ENCRYPTION_KEY` 单独存放在密码管理器中，与备份分开保管。
