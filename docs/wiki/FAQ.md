...
- **清理已归档源目录**：在设置页存储面板点该按钮（`POST /api/system/purge-archived-sources`）。会核对本冷两端 GID，**跳过 pending / downloading 任务**，只删除已有完好冷库 CBZ 对应的热目录散图。
...
- **修复**：扫描完成后标红列出异常项，点「修复」或「全选并修复」。**只重下损坏页与缺失页**，完好的页不重下，画质沿用原档位。
