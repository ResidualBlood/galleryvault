...
- **Archive / Cold storage**: located right next to library roots; left blank (example path `/archive`) to disable, with auto-archiving downloads and deleting source disabled by default. Archived CBZ files in cold storage are always named using the English title `gid-gallery.title.cbz` (English-only filename aids cross-system compatibility and cold backups) and do not follow the `download_title` setting.
