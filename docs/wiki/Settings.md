...
- **归档 / Archive**：设置项位于库根目录旁；支持配置多个归档根目录（多行文本，每行一个容器内路径如 `/archive`、`/archive2`），留空则不开启；旧配置单路径 `cold_storage_root` 会在读取时自动升级为列表。归档写入时自动挑选剩余空间 ≥ 预估体积 × 1.2 且剩余可用空间最大的根写入；库扫描会自动并入全部归档根。默认不自动归档下载、归档后默认不删源。冷库归档 CBZ 文件名固定使用英文标题 `gid-gallery.title.cbz`（始终英文更利于跨系统与冷备），不跟随 `download_title` 设置。**已归档源目录异步反向清理（`purge-archived-sources`）**：当本地画廊已成功生成有效冷存储（CBZ/EPUB）后，可通过设置页一键触发异步反向清理，释放热存储下载目录空间；具备严格的多来源 GID 校验与排他性防护机制，自动拦截处于 pending/downloading 状态的活跃任务以防误删；清理进度由 `TaskManager` 托管并在活动日志（Logs）实时展示进度条，同时由 `StorageUsageTracker` 实时统计并扣减磁盘物理用量。
- **磁盘用量**：设置页展示 library / downloads / cache 用量，以及库内体积最大的 10 本。库体积直接统计 DB `storage_size`，挂载卷读取 `disk_usage`，downloads/cache 使用进程内快照与增量更新并由后台定期 `du` 校准，打开设置页亚秒返回而不再同步扫描全盘。目录不存在该项为 0。
...
- **下载高级选项（折叠）**：H@H 网络、`max_pages`、归档页数阈值、定时扫描大画廊走归档下载。慢速 H@H 节点看门狗：**单图最大耗时（秒）**、**慢速预热窗口（秒）**、**单图最低速度（KB/s）**——超过总时长预算、或预热窗口后平均速度低于下限的单张图片会被中断并按退避重试，不再长时间拖住整个画廊。支持后台轻量探针自动探测恢复，探测间隔受环境变量 `GV_CHALLENGE_PROBE_INTERVAL`（默认 600 秒）控制。
- **标签**：
...
- **Telegram（折叠）**：bot token、chat ID、允许的 user ID，**发送测试消息**验证；**通知级别**（汇总 / 即时 / 仅失败 / 关闭）与**通知语言**（中文 / English）——下载、扫库、收藏夹检查、302 临时挑战告警（🚨 触发/✅ 恢复）与 bot 回复统一用该语言发送，消息采用 Telegram HTML 格式（标题加粗、gid 等宽），画廊标题保持原文不翻译。
