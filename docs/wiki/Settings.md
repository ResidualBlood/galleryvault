...
- **归档 / Archive**：设置项位于库根目录旁；支持配置多个归档根目录（多行文本，每行一个容器内路径如 `/archive`、`/archive2`），留空则不开启；旧配置单路径 `cold_storage_root` 会在读取时自动升级为列表。归档写入时自动挑选剩余空间 ≥ 预估体积 × 1.2 且剩余可用空间最大的根写入；库扫描会自动并入全部归档根。默认不自动归档下载、归档后默认不删源。冷库归档 CBZ 文件名固定使用英文标题 `gid-gallery.title.cbz`（始终英文更利于跨系统与冷备），不跟随 `download_title` 设置。
