...
- **归档 / Archive**：和库根在同一「资料库」分区。多行 `archive_roots`（如 `/archive`、`/archive2`），留空 = 不启用。旧字段 `cold_storage_root` 读入时升成单元素列表。写入挑「剩余空间 ≥ 预估 × 1.2 且最空」的根；扫库会扫全部归档根。默认不自动归档、归档后不删源。CBZ 路径 `{cold}/cbz/{hh}/{ii}/{gid}-{DB 英文标题}.cbz`，不跟 `download_title`；`.galleryvault.json` 写在包内。单卷上限 **500 页且 2GiB**（两者取 AND），超限打成冷目录而不是多卷 CBZ。打包与清理操作在 **管理 → 冷库归档**（`#/archive`）；设置页存储表也有「清理已归档源目录残留文件」，跳过 pending/downloading。
- **磁盘用量**：表里四行 **library / cold / downloads / cache**（路径、条目数、已用、盘剩余）。library/cold 显示画廊数与图片数，cache 显示缩略图约数。打开设置页不扫全盘。下面列出体积最大的 10 本。
- **下载常用**：根目录、并发画廊数、**单画廊并发页数**（默认 4——H@H 节点对同一出口 IP 的并发连接数有限，设太高会顶穿限制、在线路不稳时大量报连接错误；求稳就保持低值，线路干净想榨带宽再调高）、画质（普通/原图）、**归档下载质量**（归档默认档位）、**归档不可用降级为逐页下载**（默认开；归档通道无法服务该画廊时自动转逐页，不扣 GP、走 H@H）；**下载标题**：仅控制**下载热目录** `download_root` 新建文件夹命名——`japanese`（默认，`gid-日文标题`，无日文标题时 fallback 英文）/ `english`（`gid-英文标题`），与显示用「标题显示」相互独立（冷库 CBZ 文件名用数据库英文标题，不受此项影响），已下载的画廊会复用原有目录，切换设置不会改名或重复下载。
...
| 访问控制级别 | 场景 | 包含的操作与机制 |
| --- | --- | --- |
| **必须访问 ExHentai<br>/ E-Hentai 源站** | **发现页与在线搜索** | 支持普通搜索、热门(Popular)、关注(Watched)、排行榜(Toplist)；附带 cursor 分页与后端短期缓存 (60s/120s)。 |
| | **画廊下载 (两种模式)** | 1. **普通分页下载**：并发受限，原图/普通请求与 H@H 节点分离控制。<br>2. **Archive 归档下载**：扣除 GP 或配额，直接拉取源站打包的 cbz/zip 文件。 |
| | **标签与元数据同步** | 包含元数据 API (gdata) 批量拉取、在线分类回填、画质探测与自动回填。 |
| | **收藏夹双向管理** | 定期拉取云端分类/数量统计/列表；收藏项的增删改（包括移动分类、修改备注等）。 |
| | **探活与配额监控** | 后台 30 分钟周期 Cookie 探活、测试登录；反爬 (Challenge/509) 阻断时的自动探活恢复；查询 GP 与图像配额余额。 |
| | **新版本拉取** | 当一键更新触发新建下载任务拉取新 GID 时，走完整外访下载流程。 |
| **第三方网络外访** | **EhTag 词库** | 访问 GitHub API (`api.github.com`) 检查与拉取 EhTag 翻译数据库。 |
| | **Telegram 通知** | 访问 Telegram Bot API (`api.telegram.org`) 用于推送通知与 Bot 交互轮询（若配置）。 |
| **纯本地 / 完全离线** | **详情浏览与阅读** | 详情页只读本地 DB；阅读器只加载本地资源；完全不向源站发起请求。 |
| | **冷库与库维护** | 冷库归档打包 (CBZ) 与源清理；本地磁盘扫盘入库；OPDS 目录与 CBZ 导出；回收站。 |
| | **画廊更新初筛** | 基于本地已有收藏夹记录与已下载画廊标题做字符串规范化匹配初筛，不发起网络请求。 |
| | **本地分类自愈** | 根据本地已有的 `.galleryvault.json` 标签重推，修复被误标的分类，不访问外网。 |
| **本地优先 (Local First)** | **详情与入队** | 详情优先读 DB；下载入队准备时优先复用本地 DB/缓存元数据，缺失才查 gdata。 |
| | **封面与配额缓存** | 获取远程封面先查本地缓存目录，无文件再请求；GP 与配额在缓存有效期内不重复请求源站。 |
