...
- **Keyboard Shortcuts**:
  ...
  |---|---|---|
  | `←` / `→` | Previous / Next page | Automatically inverted in Manga (RTL) mode |
  | `Space` | Next page | Continuous smooth reading |
  ...
  | `Esc` | Exit fullscreen | Restores fit mode and toolbar display |
- **Mobile Touch Gestures**:
  ...
  - **Pinch-to-Zoom**: Continuous 1.0x to 3.5x smooth zoom; tap to reset boundaries.
...
- **Adaptive Slideshow (GIF/WebP Frame Detection)**:
  - The reader toolbar provides a "▶" play/pause button and an inline interval input (default 5 seconds, persisted to `gv_slideshow_interval`).
  - **Native Duration Probe & Anti-Ghosting Delay**: For animated images (GIF or animated WebP), the reader queries the metadata probe (`GET /api/galleries/{id}/pages/{page}/meta`) to retrieve the exact frame rate and total animation loop duration (`duration_ms`). The scheduler enforces `max(user_interval, duration_ms + 150ms)`, ensuring full loop playback before advancing to the next slide. Combined with browser-level pre-decoding (`img.decode()` and swap locks), this prevents tearing and frame ghosting.
  - Manual paging, image click, or adjusting the interval seamlessly pauses or reschedules the timer.
