...
- **Bulk & filtered deletion**:
  - Ticking gallery cards reveals a **Delete selected** action, with an option to delete corresponding files on disk (**leaving files on disk sends the row to `#/recycle` → User deleted**, restorable, see [Library Maintenance](Manage-EN));
