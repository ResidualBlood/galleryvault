...
- **Start archive**: pack ingested galleries into CBZ, picking the root with the most free space. Path is `{cold}/cbz/{hh}/{ii}/{gid}-{DB English title}.cbz` (ignores `download_title`). `.galleryvault.json` and `ComicInfo.xml` are **inside** the zip, not beside it. A volume is capped at **500 pages and 2GiB** (AND); larger galleries become `{cold}/dir/{hh}/{ii}/{gid}/` (not multi-volume CBZ). **Cancel archive** stops a run in progress.
