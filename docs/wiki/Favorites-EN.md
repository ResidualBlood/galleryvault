...
Incremental and force both skip galleries already in the local library. To pull down an already-checked folder's backlog, temporarily switch the mode to **force download** and run **Check now** (or tick **Download selected** inside `#/favorites/<favcat>`), then switch back to incremental to follow only new additions again.
